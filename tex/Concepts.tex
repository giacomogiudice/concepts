...
%
%\section{Clustering}
%Once our graph has been constructed, we can define the {\em distance} $d_{ij}$ between vertices $i$ and $j$ to be the length of the shortest path connecting them. 
%The concepts correspond to the graph's vertices, which form a metric space.
%\subsection{Rodriguez--Laio Clustering Algorithm}
%
%
%The best results were obtained with the algorithm proposed by Rodriguez \& Laio~\cite{rodriguezlaio2014},  that we refer to as the {\em RL algorithm}.
%We implement a variant of this algorithm that works with our specific metric and --- since the concept graph is very tightly connected --- we choose a weaker criterion than the robust assignation.
%
%A excellent description of the algorithm is provided in~\cite{rodriguezlaio2014}.
%Just as an overview, the key quantities defined for each vertex is the {\em local density} $\rho$  and the {\em distance to higher density} $\delta$
%\begin{subequations}
%  \begin{align}
%    \rho_i &= \sum_j \chi(d_c - d_{ij}) \\
%    \delta_i &=  \displaystyle \min_{j: \rho_j > \rho_i}d_{ij}
%  \end{align}
%  \label{eq:rhodelta}
%\end{subequations}
%where $d_c$ is the cutoff distance, that we set to 1 so as to consider only nearest neighbours.
%$\chi$ is the indicator function, such that $\chi(x) = 1$ if $x \geq 0$ and 0 otherwise.
%
%The mechanism behind the RL algorithm is the identification of the cluster centers as the points that stand out with high values of both $\delta$ and $\rho$.
%This can easily be visualized in a $\delta$~vs.~$\rho$ plot, called the {\em decision graph}.
%Once the cluster centers are identified, the rest of the points are assigned to the nearest clusters.
%As a weak assignation, we choose to eliminate points from a cluster with lower density than the lowest--density border points.
%The complete algorithm is presented in Algorithm~\ref{alg:clusterRL}.
%
%\begin{algorithm}[h]
%\setstretch{1.25}
%\DontPrintSemicolon
%\KwData{Distance matrix $d$}
%\KwResult{Membership vector $\bf{m}$}
%\BlankLine
%\Begin{
%  \For{$i = 1,\dots,n$}{
%    $\rho_i \asg \sum_j \chi(d_c - d_{ij})$\;
%    $\delta_i \asg  \displaystyle \min_{j: \rho_j > \rho_i}d_{ij}$\; 
%    \If{$\delta_i \in \{ \emptyset, \infty \}$}{
%      $\delta_i \asg d_{\max}$
%    }
%  }
%  ${\cal C} \asg N_{C}$ first indexes from ${\rm sort}(\rho_i \delta_i)$\;
%  ${\cal B}_{i,\dots,N_c} \asg \emptyset$\;
%  \For{$i = 1,\dots,n$}{
%    $m_i \asg  \displaystyle \arg \min_{j \in {\cal C}} d_{ij} $ \bf{if} $\displaystyle \min_{j \in {\cal C}} d_{ij} < \infty$\;
%    \If{$\displaystyle \min_{j \in {\cal C} \smallsetminus m_i} d_{ij} < d_c$}{
%    	add $i$ to ${\cal B}_{m_i}$\;
%    }
%    
%  }
%  \For{$c \in {\cal C}$}{
%    $\rho_l \asg \displaystyle \min_{j \in {\cal B}_c} \rho_j$\;
%    \For{$i : m_i = c, \rho_i \leq \rho_l$ }{
%     $m_i = 0$\;
%   }
%  }
%
%  }
%\caption{Modified RL algorithm used for clustering.}
%\label{alg:clusterRL}
%\end{algorithm}
%
%\subsection{Full Dataset}
...
%%\begin{figure}
%  \centerline{
%    \includegraphics[height=7cm,width=0.6\textwidth,keepaspectratio]{../img/decision.pdf}
%    \includegraphics[height=7cm,width=0.4\textwidth,keepaspectratio]{../img/distribution_distance.pdf}
%  }
%  \caption{ {\em Left}: decision graph for the subset of concepts. Highlighted points correspond to cluster centers. {\em Right}: distribution of distances between vertices on the graph.}
%  \label{fig:decision}
%\end{figure}
%
%Binarizing these couplings for $J_\bot = 0.5 > t_1$ we obtain the decision graph of fig.~\ref{fig:decision}.
%We recognize 5 different cluster centers and, using weak assignation, we find clusters of different sizes presented in fig.~\ref{fig:clusters2-3}-\ref{fig:clusters4-5}, with highlighted cluster centers. 
%The different clusters generally contain concepts that are semantically close to each other: for example the {\em Chemical potential} cluster groups concepts related to condensed matter physics.
%The {\em Scalar field} cluster contains a large number of terms used in high energy physics tightly tied together. 
%The rest of the clusters regard astrophysics.
%The {\em Milky Way} cluster contains words specific to the observation of celestial bodies, while {\em Star} cluster contains more generic conceps like {\em Velocity}, {\em Experimental Data} and {\em Wavelength}.
%The {\em Luminosity} cluster is not presented as it has over 400 concepts but regroups concepts mostly used in cosmology, astronomy and particle physics. 
%
%
%Using strong cluster assignation, as used in~\cite{rodriguezlaio2014}, we have a very selective criterion on this particular network that leads to clusters of very few concepts. 
%An empirical study of the graph vertexes show resonable connections, suggesting that the ontology constructed has a certain validity. 
%However the difficulty lies in the interpretation of the results.
%We believe our clustering techniques are not particularly apt at studying the concept network, since it is very interconnected and the definition of a cluster is not trivial and dependent on the application one is interested in. For example for a semantic classification an approach based on the minimum spanning trees might be more fruitful.
%
%\newpage
%\begin{figure}[h]
%  \centering{
%   \includegraphics[width=\textwidth]{../matlab/clusters/cluster2.pdf}
%   \includegraphics[width=\textwidth]{../matlab/clusters/cluster3.pdf}
%  }
%  \caption{{\em Milky Way} cluster ({\em top}) \& 3 ({\em Scalar field}) cluster ({\em bottom}).}
%  \label{fig:clusters2-3}
%  
%\end{figure}
%\newpage
%\thispagestyle{empty}
%\begin{figure}[h]
%  \centering{
%   \includegraphics[width=\textwidth]{../matlab/clusters/cluster4.pdf}
%   \includegraphics[width=\textwidth]{../matlab/clusters/cluster5.pdf}
%  }
%  \caption{{\em Chemical potential} cluster ({\em top}) \& {\em Star} cluster ({\em bottom}).}
%  \label{fig:clusters4-5}
%  \thispagestyle{empty}
%\end{figure}
%\newpage
